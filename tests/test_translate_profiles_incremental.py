...
from translate_profiles import filter_files_since, parse_since


class TranslateProfilesIncrementalTests(unittest.TestCase):
    ...
    def test_filter_files_since_uses_source_file_mtime(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            old_file = tmp / "player_1_old.json"
            new_file = tmp / "player_2_new.json"
