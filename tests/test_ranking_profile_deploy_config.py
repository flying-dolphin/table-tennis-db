...
class RankingProfileDeployConfigTests(unittest.TestCase):
    def test_runtime_bundle_includes_ranking_profile_import_dependencies(self):
        upload_script = (ROOT / "deploy" / "server" / "update_rankings_profiles.sh").read_text(encoding="utf-8")
    ...
    def test_data_update_script_validates_profile_payload_before_backup_and_imports(self):
        update_script = (ROOT / "deploy" / "server" / "update_rankings_profiles.sh").read_text(encoding="utf-8")

        validation_command = "scripts/db/import_players.py --dir '${profile_dir}' --validate-only"
        validation_pos = update_script.index(validation_command)
        backup_pos = update_script.index("\n    backup_remote_database\n")
        imports_pos = update_script.index("==> Running remote imports")

        self.assertLess(validation_pos, backup_pos)
        self.assertLess(validation_pos, imports_pos)
