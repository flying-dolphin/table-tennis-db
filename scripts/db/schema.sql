...
-- ============================================================================
-- 个人对阵明细（来自 completed_matches.json，每条团体赛 match 内的单打/双打）
-- ============================================================================

CREATE TABLE IF NOT EXISTS event_schedule_match_games (
    game_id             INTEGER PRIMARY KEY AUTOINCREMENT,
    schedule_match_id   INTEGER NOT NULL,
    game_order          INTEGER NOT NULL,   -- 1-based，场内第几盘
    player_a_name       TEXT,              -- 对应 side A 球员
    player_b_name       TEXT,              -- 对应 side B 球员
    player_a_id         INTEGER,           -- 可空：未匹配
    player_b_id         INTEGER,
    game_score          TEXT,              -- "3-1"（以 side A 视角）
    set_scores          TEXT,              -- "11-9,8-11,11-7,11-6"（以 side A 视角）
    ...
    CHECK (winner_side IN ('A', 'B') OR winner_side IS NULL),
    UNIQUE(schedule_match_id, game_order)
);

CREATE INDEX IF NOT EXISTS idx_event_schedule_match_games_match
    ON event_schedule_match_games(schedule_match_id);
