...
def player_output_filename(player_name: str, player_id: Any = None) -> str:
    safe_name = sanitize_filename(player_name)
    safe_player_id = str(player_id or "").strip()
    if safe_player_id:
        return f"player_{sanitize_filename(safe_player_id)}_{safe_name}.json"
...
def run(args: argparse.Namespace) -> int:
    ...
    with sync_playwright() as p:
        ...
        for i, player in enumerate(players, start=1):
            ...
            orig_file = output_orig_dir / player_output_filename(player_name, player_id)
            # 从 orig 目录读取现有数据
            if orig_file.exists():
                try:
            ...
            try:
                ...
                if total_events_in_json == 0 and not args.allow_empty:
                    raise RuntimeError(
                ...
                if total_events_in_json == 0:
                    # --allow-empty：同名消歧证据准备等场景下，某成员窗口内无比赛是
                    # 合法结果。写出空证据文件（避免下次重复抓取）并继续。
                    logger.warning(
                        "No events for %s (%s) since %s; writing empty evidence (--allow-empty).",
                        player_name, country_code, from_date,
                    )
                # 保存原始数据到 orig 目录
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--allow-empty",
        action="store_true",
        help="Treat 'no events since from-date' as a valid empty result (write empty evidence, exit 0) "
             "instead of a fatal error. Used by same-name disambiguation evidence prep.",
    )
    return parser
