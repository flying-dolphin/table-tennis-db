...
EVENT_TRANSLATOR: DictTranslator | None = None
...
def translate_file(file_path: Path, cn_file: Path, args: argparse.Namespace) -> int:
    ...
    for i, event in enumerate(events):
        ...
        # Name: try dict
        name_zh = translate_event_name_dict_only(name, EVENT_TRANSLATOR)
    ...
    dict_translated = len(base_events) - skipped - len(missing_names)
    logger.info(
        "Name translation plan: %d skipped (existing), %d dict, %d missing",
        skipped, dict_translated, len(missing_names),
    )
    if missing_names:
        logger.warning("Dictionary missing %d event names: %s", len(missing_names), missing_names[:5])
...
def run(args: argparse.Namespace) -> int:
    global DICTIONARY, EVENT_TRANSLATOR
    ...
    EVENT_TRANSLATOR = DictTranslator(DICT_PATH)
