...
def move_mouse_to_locator(page: Any, locator: Any) -> None:
    """P1: 模拟贝塞尔曲线鼠标移动轨迹后点击，替代直接 locator.click()"""
    try:
...
def type_like_human(page: Any, locator: Any, text: str) -> None:
    """P1: 移动鼠标到输入框后再输入，避免直接 locator.click() 的机械特征"""
    move_mouse_to_locator(page, locator)
    # fill("") 直接清空 DOM value，比 Control+a+Backspace 更可靠，
    # 防止切换 player 时搜索框保留上一个 player 的内容
    locator.fill("")
...
def collect_events_with_pagination(
    page: Any, from_year: int, max_pages: int = 50
) -> list[dict[str, Any]]:
    """遍历所有 event 分页，按时间倒序遍历直到遇到早于 from_year 的赛事则停止。"""
    ...
    for page_num in range(max_pages):
        ...
        for event in events:
            try:
                event_year = int(event.get("year", 0))
            ...
            if event_year > 0 and event_year < from_year:
                reached_cutoff = True
                logger.info(
                    "Reached event year=%s < from_year=%s on page %s, stopping pagination",
                    event_year, from_year, page_num + 1,
                )
                break
...
def capture_json_responses_for_page(page: Any, visit: callable) -> list[dict[str, Any]]:
    ...
    try:
        ...
        # patchright 未实现 page.off()，使用底层 remove_listener()
        try:
...
def ensure_logged_in(page: Any, delay_cfg: DelayConfig, storage_state_file: Path, init_session: bool) -> None:
...
def scrape_player(
    ...
    events = collect_events_with_pagination(page, from_year=from_date.year)
    logger.info("Found %s events for %s since %s", len(events), player_name, from_date)

    # 翻页结束后记录事件列表 URL，作为 detail 页面的 Referer
    events_list_url = page.url

    # 加载已抓取的 event keys，避免重复抓取
    already_scraped = _scraped_event_keys(player_output)
    if already_scraped:
        logger.info("Loaded %s already-scraped events to skip", len(already_scraped))
...
def _scraped_event_keys(player_output: dict[str, Any] | None) -> set[tuple[str, str]]:
    """提取已抓取的 (event_year, event_name) 组合，用于跳过重复抓取。"""
    if not player_output:
        return set()
    keys: set[tuple[str, str]] = set()
    for year_data in player_output.get("years", {}).values():
        for event in year_data.get("events", []):
            keys.add((str(event.get("event_year", "")), event.get("event_name", "")))
    return keys
...
def run(args: argparse.Namespace) -> int:
    ...
    with sync_playwright() as p:
        ...
        for i, player in enumerate(players, start=1):
            ...
            out_file = output_dir / f"{sanitize_filename(player_name)}_{rank}.json"
            if out_file.exists():
                try:
            ...
            if checkpoint.is_done(ck) and not args.force:
                logger.info("Skip completed: %s", ck)
            else:
                logger.info("Scraping %s since %s", player_name, from_date)
                try:
                    ...
                    # 最终写入确保元数据（captured_at/from_date）也已更新
                    merge_player_data(player_output, player_data)
                    save_json(out_file, player_output)
                    checkpoint.mark_done(ck)
                    logger.info("Completed %s (%s events)", player_name, len(player_data.get("events", [])))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--players-file", default="data/women_singles_top100.json")
    parser.add_argument("--output-dir", default="data/matches_complete_v2")
    ...
    parser.add_argument("--checkpoint", default="data/checkpoints/ittf_checkpoint_v2.json")
