...
export default function RankingsPageClient({
  ...
  const loadPlayers = useCallback(async (offset: number, nextSortBy: SortField, isInitial = false) => {
    ...
    try {
      const res = await fetch(`/api/v1/rankings?sort_by=${nextSortBy}&limit=20&offset=${offset}`);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        ...
        void loadPlayers(players.length, sortBy);
      },
      {
  ...
  }, [hasMore, loading, loadingMore, loadPlayers, players.length, sortBy]);
