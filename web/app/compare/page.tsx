  ...
  const MetricRow = ({ label, valA, valB, higherIsBetter = true, isPercentage = false }: any) => {
    ...
    return (
      <div className="flex flex-col py-3 border-b border-border-subtle last:border-0">
        <div className="text-[12px] text-text-tertiary text-center mb-1 font-medium">{label}</div>
        <div className="flex items-center justify-between px-2">
          <div className={cn(
            "flex-1 text-center text-[16px] font-bold tabular-nums",
            !isEqual && (isHigherA === higherIsBetter ? "text-brand-deep scale-110" : "text-text-secondary opacity-60")
          )}>
            {format(valA)}
          </div>
          ...
          <div className={cn(
            "flex-1 text-center text-[16px] font-bold tabular-nums",
            !isEqual && (isHigherB === higherIsBetter ? "text-brand-deep scale-110" : "text-text-secondary opacity-60")
          )}>
            {format(valB)}
          </div>
        </div>
  ...
  return (
    <main className="min-h-screen pb-20 bg-page-background">
      ...
      <div className="bg-white/80 backdrop-blur-md sticky top-0 z-30 border-b border-border-subtle px-4 py-3 flex items-center justify-between">
        ...
        <h1 className="text-[17px] font-bold text-text-primary">球员对比</h1>
        <Link href="/rankings" className="text-[13px] text-brand-deep font-medium">更换</Link>
      </div>

      <div className="px-4 py-6">
        {/* Player Cards */}
        <div className="flex items-center gap-3 mb-8">
          <Link href={`/players/${pA.slug}`} className="flex-1 bg-white rounded-[24px] p-4 flex flex-col items-center shadow-sm border border-white">
            <PlayerAvatar player={pA} size="lg" />
            <h2 className="mt-3 text-[16px] font-bold text-text-primary text-center leading-tight">
              {pA.nameZh || pA.name}
            </h2>
            <span className="mt-1 text-[11px] font-medium text-text-tertiary bg-page-background px-2 py-0.5 rounded-full uppercase">
              {pA.countryCode}
            </span>
          </Link>
          ...
          <Link href={`/players/${pB.slug}`} className="flex-1 bg-white rounded-[24px] p-4 flex flex-col items-center shadow-sm border border-white">
        ...
        <div className="bg-white rounded-[24px] p-5 shadow-sm border border-white mb-6">
          <div className="flex items-center gap-2 mb-4">
            <Zap size={18} className="text-brand-deep" />
            <h3 className="text-[15px] font-bold text-text-primary">历史交手</h3>
          </div>
          
          <div className="flex items-center justify-between mb-2">
            <div className="flex flex-col items-center flex-1">
              <span className="text-[28px] font-black text-brand-deep leading-none">{data.headToHeadSummary.playerA.wins}</span>
              <span className="text-[12px] text-text-tertiary mt-1">胜</span>
            </div>
            <div className="px-4 text-[12px] font-bold text-text-tertiary bg-page-background rounded-full py-1">
              共 {data.headToHeadSummary.totalMatches} 场
            </div>
            <div className="flex flex-col items-center flex-1">
              <span className="text-[28px] font-black text-text-secondary leading-none">{data.headToHeadSummary.playerB.wins}</span>
              <span className="text-[12px] text-text-tertiary mt-1">胜</span>
            </div>
          </div>
        ...
        {/* Stats Metrics */}
        <div className="bg-white rounded-[24px] p-2 shadow-sm border border-white mb-6">
          <div className="px-3 pt-3 pb-1 flex items-center gap-2">
            <Globe size={18} className="text-brand-deep" />
            <h3 className="text-[15px] font-bold text-text-primary">核心指标</h3>
          </div>
        ...
        {/* H2H Match List */}
        <div className="bg-white rounded-[24px] p-2 shadow-sm border border-white">
          <div className="px-3 pt-3 pb-3 flex items-center gap-2">
            <Trophy size={18} className="text-brand-deep" />
            <h3 className="text-[15px] font-bold text-text-primary">交手记录</h3>
          </div>
          
          <div className="flex flex-col">
            {data.headToHeadMatches.length === 0 ? (
              <div className="py-10 text-center text-text-tertiary text-[14px]">暂无交手记录</div>
            ) : (
                ...
                return (
                  <div key={match.matchId} className="flex items-center px-3 py-3 border-b border-border-subtle last:border-0">
                    <div className="flex-1 overflow-hidden">
                      <div className="text-[11px] text-text-tertiary mb-0.5 truncate">
                    ...
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        "w-6 h-6 rounded flex items-center justify-center text-[11px] font-bold transition-all",
                        isWinnerA ? "bg-brand-deep text-white" : "bg-page-background text-text-tertiary"
                      )}>
                        {isWinnerA ? "胜" : "负"}
                      </div>
                      <div className="text-[15px] font-black tabular-nums min-w-[36px] text-center">
                        {match.matchScore}
                      </div>
                      <div className={cn(
                        "w-6 h-6 rounded flex items-center justify-center text-[11px] font-bold transition-all",
