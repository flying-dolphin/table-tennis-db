...
live / completed`）；cron 则以 `match_details` 替代 `completed`（抓取比赛详情并导入
live + completed 表）。`session_schedule` 是人工日程，不在默认里，只通过上表
...
- `standings`：Main Draw 前刷新（`session_start + 3h`）
- `brackets`：Main Draw 前及比赛阶段刷新（`session_start + 3h`）
- **session 刷新窗口**（`live` + `match_details`）：
  - `live`：每 **10 分钟**刷新一次比分（导入 `live` 表）
  - `match_details`：每 **30 分钟**抓取一次比赛详情（同时导入 `live` + `completed` 表）
