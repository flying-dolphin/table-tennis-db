...
- **头像缩略图走 bind-mount，不进镜像**：`web/public/images/{avatar-thumbs,avatar-full-thumbs}` 本质是数据而非代码。`.dockerignore` 已排除头像源图、裁剪图和缩略图目录，运行时只由 `docker-compose.yml` 把宿主 `${ITTF_DATA_DIR}/web_assets/{avatar-thumbs,avatar-full-thumbs}` 以只读 bind-mount 挂回 `/app/web/public/images/` 对应路径。线上页面只读取缩略图目录；`avatars` 和 `crops` 只作为离线生成缩略图的源数据，不挂进 Web 容器。
...
代价是首次部署多一步生成缩略图并复制 `web/public/images/{avatar-thumbs,avatar-full-thumbs}` 到 `data/web_assets/`，见 6.1 节。
