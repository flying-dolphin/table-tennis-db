...
- [ ] 已执行 `/opt/ittf-ops/install_current_event_crontab.sh <event_id>` 并成功安装 `ITTF current-event refresh` 托管区块
...
不会。它也是服务端运行时从 `/app/data/player_country_history.json` 读取的宿主数据卷文件，用于详情页回写历史国家。部署时要和数据库一起同步到 `${ITTF_DATA_DIR}`，否则对应球员的历史国家不会回写。
