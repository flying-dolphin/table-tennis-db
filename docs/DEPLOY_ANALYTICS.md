...
- **环境变量分三类**（重要）：
  ...
  - **运行时 [runtime]**：`SMTP_PASS`、`SENTRY_DSN`、`ITTF_DATA_DIR` 等——只在 `up` 时通过宿主环境注入容器，不在镜像中
- **CSP 策略由 Next.js 自动生成**：`web/next.config.ts` 的 `buildCsp()` 会根据环境变量自动把 Umami / Clarity / Sentry 的域名加入白名单，无需手动维护。
...
6. 服务器 A      pull 镜像 → 起容器 + nginx
7. 本地服务器    部署抓取 + rsync + Sentry Cron
8. 验证清单      逐项打勾
```
...
docker compose 通过 `secrets:` 段配合 `environment:` 来源做同样的事情。
