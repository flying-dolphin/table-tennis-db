...
| 数据 | 频率 | 方式 |
|------|------|------|
| 服务器 A SQLite | 手动执行 `event_refresh.sh` 时 | `data/db/backups/` 30 天 rotate（`deploy/server/event_refresh.sh` 已包含） |
...
| `deploy/server/update_event_runtime.sh` | 发布当前赛事刷新 runtime，可选安装赛事 cron |
...
| `deploy/server/upload_runtime.ps1` | 旧 PowerShell runtime 上传脚本；优先使用 `update_event_runtime.sh` |
