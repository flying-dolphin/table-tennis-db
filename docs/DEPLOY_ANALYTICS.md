...
### 8.2.1 为什么不能只同步 `ittf.db`

本项目代码在 [`web/lib/server/db.ts`](../web/lib/server/db.ts) 中显式启用了 `WAL` 模式：
...
- `ittf.db` 时间戳变化不明显
- `ittf.db-wal` 明显更新，甚至比主库还大

这是正常的，不代表导入失败，而是说明最新事务还停留在 `WAL` 文件里。如果此时只同步 `ittf.db`：

- 线上会丢失还没 checkpoint 回主库的最新数据
- 本地和线上看起来会“明明刚导入过，但数据不一致”

正确做法是二选一：

1. **推荐**：对本地库执行 `sqlite3 data/db/ittf.db ".backup data/db/ittf.incoming.db"`，同步这个单文件快照
2. 先停掉所有写库进程，执行 `PRAGMA wal_checkpoint(TRUNCATE);`，再同步 `ittf.db`

不建议在数据库仍被进程持有并且还可能继续写入时，仅靠 `scp ittf.db` 或同时复制 `ittf.db` / `ittf.db-wal` 来做发布；这样拿到的未必是同一时刻的一致性快照。
...
如果服务器上缺这个文件，从仓库或抓取机同步 `data/wtt_raw/{event_id}/` 后再刷新页面；这类 JSON 当前属于服务端运行时读取的数据，不是镜像内置资产。
