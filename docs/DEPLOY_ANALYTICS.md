...
- **赛事刷新直接放服务器 A**：`scrape_event_results_daily.py` 调的是 WTT CMS 公开接口，纯 HTTP，不依赖浏览器或人工过 Cloudflare，因此适合在 Linux 服务器定时跑。
- **rankings 仍保留在 Windows 开发机手动执行**：当前 rankings 抓取前仍需先打开 CDP 浏览器并手工通过 Cloudflare 风控，不适合伪装成无人值守任务。
- **服务器 A 现在需要轻量 Python 环境**：仅用于赛事刷新脚本；Playwright / Chromium 仍不建议装在服务器 A 上。
...
- `scripts/scrape_event_results_daily.py` 调用的是 WTT 公开 CMS API，纯 HTTP，无需浏览器
- `scripts/run_rankings.py` 当前仍依赖你先在 Windows 上启动 CDP 浏览器并手工通过 Cloudflare 风控

### 8.2 服务器 A 自动赛事刷新

完整步骤详见 [`deploy/server/README.md`](../deploy/server/README.md)。

数据流：

```
服务器 A：
  cron (可选 sentry-cli monitors run)
    └─► deploy/server/event_refresh.sh
          ├─ sqlite3 .backup 当前 /opt/ittf-data/db/ittf.db
          ...
          └─ runtime/python/refresh_event_results_daily.py
                 ├─ 刷新 /opt/ittf-data/wtt_raw/{event_id}/*.json
                 └─ 重导 event_schedule_* 表
```
...
/opt/ittf-ops/
  ...
  runtime/
    data/stage_round_mapping.json
    python/backfill_events_calendar_event_id.py
    python/import_session_schedule.py
    python/import_wtt_event.py
    python/refresh_event_results_daily.py
    python/scrape_wtt_event.py
```
...
- 服务器上**不需要完整仓库**
- 只需要上传上面这些文件
- 数据库和 raw JSON 单独放在 `ITTF_DATA_DIR` 指向的数据目录
...
1. 在服务器上创建 `runtime/data`、`runtime/python`
2. 上传最小运行包需要的所有文件
...
如果服务器上缺这个文件，先在服务器 A 执行 `./deploy/server/event_refresh.sh`；这类 JSON 当前属于服务端运行时读取的数据，不是镜像内置资产。
...
| `deploy/server/README.md` | 服务器赛事自动刷新 runbook |
