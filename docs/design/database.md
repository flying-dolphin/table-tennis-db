...
points_rules 独立维护，通过 event_category 文本名称与 event_categories 松耦合
```
...
CREATE TABLE points_rules (
    ...
    event_category      TEXT NOT NULL,         -- 赛事分类名，如 "WTT Grand Smash"
    event_category_zh   TEXT,                  -- 中文名
...
- `event_category` 使用文本名称而非 FK，便于独立维护和跨版本对比
...
步骤：
1. 初始化：执行 DDL 建表
2. 字典表：导入 event_categories、event_type_mapping、sub_event_types、points_rules
