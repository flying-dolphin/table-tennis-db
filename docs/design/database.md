...
- `points_rules_by_category` 仅保留 schema 设计，导入脚本延后到后续实现计划
...
| 积分规则 | schema 已预留 | `points_rules_by_category` 延后到后续实现 |
...
points_rules_by_category 独立维护，通过 category_id 与 event_categories 关联
```
...
| `docs/ITTF-Ranking-Regulations-20260127.md` | `points_rules_by_category` | 积分规则原始文档（行式存储，按名次逐条录入） |
