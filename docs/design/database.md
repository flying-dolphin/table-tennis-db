...
event_categories 1──N points_rules_by_category
...
### 3.12 points_rules_by_category — 积分规则

每个赛事分类在不同比赛格式下的积分值，来源于 `docs/ITTF-Ranking-Regulations-20260127.md`。

```sql
CREATE TABLE points_rules_by_category (
    id                      INTEGER PRIMARY KEY AUTOINCREMENT,
    category_id             INTEGER NOT NULL,   -- -> event_categories.id
    format_type             TEXT NOT NULL,      -- Singles / Doubles / Mixed Doubles / Teams
    w_points                INTEGER,
    f_points                INTEGER,
    sf_points               INTEGER,
    qf_points               INTEGER,
    ...
    r128_points             INTEGER,
    qual_points             INTEGER,
    qual_4th_points         INTEGER,
    qual_3rd_points         INTEGER,
    qual_2nd_points         INTEGER,
    qual_1st_points         INTEGER,

    FOREIGN KEY (category_id) REFERENCES event_categories(id) ON DELETE CASCADE,
    UNIQUE(category_id, format_type)
);

CREATE INDEX idx_points_rules_category ON points_rules_by_category(category_id);
```
...
| `docs/ITTF-Ranking-Regulations-20260127.md` | `points_rules_by_category` | 分类积分规则原始文档 |
...
步骤：
1. 初始化：执行 DDL 建表
2. 字典表：导入 event_categories、event_type_mapping、sub_event_types、points_rules_by_category
...
8. 日历：遍历 events_calendar/cn/*.json → events_calendar
   - 同样执行 event_type / event_kind 到 event_categories 的标准化映射
9. 数据对账：生成摄入报告
   - 各表成功率
   - 未匹配记录统计
   - 分类映射覆盖率
...
- 赛事积分自动计算：`points_rules_by_category` 可驱动积分验证逻辑
