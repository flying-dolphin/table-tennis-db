...
- 特殊赛制赛事（如 `event_id=2860`）当前不能仅靠 `Final + bracket` 模型完整表达
- 这类赛事需要额外的 presentation / standings 覆盖层，详见 `docs/design/special-event-round-robin.md`
...
- 当前 `sub_events` 的冠军主要来自 `Final`
- 对于“两阶段循环赛 / standings 决冠”赛事，冠军来源需要允许来自 override 或 standings
- `event_id=2860` 不应通过伪造 `Final` 写入冠军
...
```text
events_calendar / event_schedule / WTT raw
    → backfill_events_calendar_event_id.py
    → import_session_schedule.py
    → scrape_wtt_event.py
    → import_wtt_event.py
    → event_session_schedule / event_schedule_matches / event_draw_entries*
```

说明：

- 这条链路用于 `upcoming` / `draw_published` / `in_progress` 赛事
- 当前前端已直接消费 `event_session_schedule` 与 `event_schedule_matches`
- 对进行中团体赛，页面可直接从 `event_schedule_matches` 推导小组赛积分表，不必等待历史表 `matches` / `event_draw_matches` 完整落库

特殊赛制补充：

- 对于 `event_id=2860` 这类无 `Final` 的赛事，`sub_events` 不能仅依赖第 7 步
- 需要引入 `manual_event_overrides` 作为冠军和最终排名补充来源
- `import_event_draw_matches.py` 也不应为这类赛事强行生成 knockout bracket
...
| `data/event_schedule/*.json` | 即将开赛 / 进行中赛事的按日纲要日程来源 |
| `data/wtt_raw/{event_id}/*.json` | 即将开赛 / 进行中赛事的 WTT raw 赛程、签表、结果来源 |
...
- 球员名字可能有拼写变体、别名
- `events.time_zone` 当前仍不完整，部分 upcoming / in-progress 赛事依赖 `import_wtt_event.py` 推断补齐
...
- 进行中团体赛可直接从 `event_schedule_matches` 推导小组赛积分表
- 赛事详情页可对进行中赛事补充北京时间提示
